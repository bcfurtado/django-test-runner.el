...
(ert-deftest django-test--convert-path-into-python-module ()
  (ert-info ("convert path into python module")
    (should (equal
	     (django-test--convert-path-into-python-module "project/file/test.py")
	     "project.file.test"))))

(ert-deftest django-test--current-class ()
  (ert-info ("return the current class when currently at a function")
    (with-mock
     (stub python-info-current-defun => "TestCase1.test1")
     (should (equal (django-test--current-class) "TestCase1"))))
  ...
  (ert-info ("return nil when currently not at a class or function")
    (with-mock
     ...
     (should (eq (django-test--current-class) nil)))))
